...
int main() {
    vector<long long> sizes = {20000000, 30000000, 40000000, 50000000};
    vector<int> maxValues = {100, 500, 1000, 1500};

    cout << "\n";
    cout << "--------------------------------------------------------------------------------------------------------------------\n";
    cout << "| Input Size | Max Value | CPU Sum     | GPU Sum     | CPU Time (s) | GPU Time (s) | Speedup | Efficiency |\n";
    cout << "--------------------------------------------------------------------------------------------------------------------\n";
    ...
    for (size_t i = 0; i < sizes.size(); i++) {
        ...
        chrono::duration<double> cpuTime = cpuEnd - cpuStart;
        ...
        cout << "| " << setw(10) << n
             ...
             << " | " << setw(11) << cpuSum
             << " | " << setw(11) << h_sum
             << " | " << setw(12) << fixed << setprecision(6) << cpuTime.count()
             << " | " << setw(12) << gpuTimeMs / 1000.0
